...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let device = if args.iter().any(|a| a == "spaceorb") {
        "spaceorb"
    } else {
        "spaceball"
    };
    ...
    if device == "spaceorb" {
        ...
        eprintln!("Initialized SpaceOrb. Reading bytes (Ctrl-C to quit):\n");
        dump_bytes(orb.bytes(), cfg);
    } else {
        let mut sb = Spaceball::open(&path)?;
        eprintln!("Initialized Spaceball. Reading bytes (Ctrl-C to quit):\n");
        dump_bytes(sb.bytes(), cfg);
    }
    Ok(())
}

fn dump_bytes(bytes: impl Iterator<Item = Result<u8, std::io::Error>>, cfg: HexConfig) {
